 ...
 */
 ...
 //Detection en utilisant le needle model 3D (à initialiser préalablement)
 // Puis simule, à chaque nouvelle image, des particules (=courbes représentant l'aiguille)
 // qui varient autour des points de contrôle de l'image précédente
 // On pondère ensuite toutes ces particules avec la réalité
 // (somme des intensités des pixels de chaque particule simulée : blanc = poids élevé car aiguille détecté en blanc dans l'image echo)
 // Puis on prend la moyenne pondérée de ces particules pour définir les points de controle sur la nouvelle image


class usNeedleTrackerSIR3D
{
 public:
  /**
   * Default constructor.
   ...
   */
  void init(const usImagePostScan3D<unsigned char>& vol,double origin[3], double fov, unsigned int nPoints,

...
class usNeedleTipDetector3D {
 public:
  void init(const usImage3D<unsigned char>& V, unsigned int nbCandidates, double varThreshold);
  bool detect(const usImage3D<unsigned char>& V);

 private:
  usImage3D<unsigned char> m_lastVolume;
  usImage3D<unsigned int> m_difference;

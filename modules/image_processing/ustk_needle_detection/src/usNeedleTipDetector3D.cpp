...
#include <visp3/ustk_needle_detection/usNeedleTipDetector3D.h>

#include <visp3/ustk_core/usImage3DProcessing.h>
#include <visp3/ustk_needle_detection/usNeedleDetectionTools.h>

void usNeedleTipDetector3D::init(const usImage3D<unsigned char>& V, unsigned int nbCandidates,
				 double varThreshold)
{
  m_lastVolume = V;
...
bool usNeedleTipDetector3D::detect(const usImage3D<unsigned char>& V)
{
  usImage3DProcessing::absoluteDifference(V, m_lastVolume, m_difference);
  m_intThreshold = usNeedleDetectionTools::getThresholdedCoordinates(m_difference,

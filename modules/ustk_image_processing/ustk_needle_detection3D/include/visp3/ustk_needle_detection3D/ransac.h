...
public:
   ...
   * Constructor.
   */
  Ransac(unsigned int s, double costThresh, double failThresh);
  /**
   ...
   */
  int run(vpMatrix &x,
	  vpMatrix &bestModel, vpMatrix &bestMSS, vpColVector &bestInliers, int n_inliers_p);
  /**
   * Returns the permutation sorting the vpColVector C.
   */
  static unsigned int *argSort(const vpColVector &C, bool ascent = 1);
  /**
   * Checks wether a set of indexes is degenerated. Returns true iff at least two indexes are equal.
   */
  static bool isDegenerated(unsigned int *ind, unsigned int npts);
  /**
   * Cost function.
   */

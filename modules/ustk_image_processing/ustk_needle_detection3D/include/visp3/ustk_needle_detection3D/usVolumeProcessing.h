 ...
 *
 * This namespace contains processing tools (derivative, filtering...) for the usImage3D class.
 */
namespace usVolumeProcessing
{
  /**
   ...
   */
  template<class T> VISP_EXPORT
    double applyFilter(const usImage3D<T> &Src, const usImage3D<double> F,
   ...
   */
  template<class T> VISP_EXPORT
    void applyFilter(const usImage3D<T> &Src, usImage3D<double> &Dst,
         const usImage3D<double> F);
   ...
   */
  template<class T> VISP_EXPORT
    void derivativeX_2(const usImage3D<T> &Src, usImage3D<double> &Dst,
   ...
   */
  template<class T> VISP_EXPORT
    void frangi(const usImage3D<T> &Src, usImage3D<double> &Dst,
		double a, double b, double c);

  /**
   * Compute the norm of a vector image.
   */
  void VISP_EXPORT norm(const usImage3D<vpColVector> &Src, usImage3D<double> &Dst);
   ...
   */
  template<class T1, class T2> VISP_EXPORT
   ...
   */
  usImage3D<double> VISP_EXPORT getDerivativeFilterX(double sigma, unsigned int size);

  /**
   * Generate a Gaussian derivative filter in the y-direction.
   */
  usImage3D<double> VISP_EXPORT getDerivativeFilterY(double sigma, unsigned int size);

  /**
   * Generate a Gaussian derivative filter in the z-direction.
   */
  usImage3D<double> VISP_EXPORT getDerivativeFilterZ(double sigma, unsigned int size);

  /**
   * Generate a Gaussian derivative filter in the x-direction.
   */
  usImage3D<double> VISP_EXPORT getDerivativeFilterXX(double sigma, unsigned int size);

  /**
   * Generate a Gaussian derivative filter in the y-direction.
   */
  usImage3D<double> VISP_EXPORT getDerivativeFilterYY(double sigma, unsigned int size);

  /**
   * Generate a Gaussian derivative filter in the z-direction.
   */
  usImage3D<double> VISP_EXPORT getDerivativeFilterZZ(double sigma, unsigned int size);

  /**
   * Generate a Gaussian derivative filter in the x-direction.
   */
  usImage3D<double> VISP_EXPORT getDerivativeFilterXY(double sigma, unsigned int size);

  /**
   * Generate a Gaussian derivative filter in the y-direction.
   */
  usImage3D<double> VISP_EXPORT getDerivativeFilterXZ(double sigma, unsigned int size);

  /**
   * Generate a Gaussian derivative filter in the z-direction.
   */
  usImage3D<double> VISP_EXPORT getDerivativeFilterYZ(double sigma, unsigned int size);

  /**
   * Compute the photometric barycenter of the volume.
   */
  void VISP_EXPORT computeBarycenter(const usImage3D<unsigned char> &V, double &xc, double &yc, double &zc);
  ...
  template<class T>
    void derivativeX_2(const usImage3D<T> &Src, usImage3D<double> &Dst,
		       unsigned int filter_size, double sigma)
    {
      usImage3D<double> F = getDerivativeFilterX(sigma, filter_size);
